/**
 * AIBTC Trading Competition Tools (read-only, NOT REGISTERED)
 *
 * The competition has concluded. Trade submission and the DEX allowlist were
 * removed, and these two remaining read tools are no longer exposed over MCP:
 * `registerCompetitionTools` is intentionally not called from `src/tools/index.ts`.
 *
 * The module is kept because the standing/history endpoints still exist and the
 * P&L math (`computeCampaignStats`) is the reference implementation. Re-expose by
 * restoring the import + call in `src/tools/index.ts`; the tests here exercise the
 * module directly, so it stays compiled and verified while unwired.
 *
 * Tools:
 * - competition_status        — Standing + mark-to-current P&L for an address
 * - competition_list_trades   — Paginated trade history
 *
 * Reads are over public addresses, no signing. Rate-limited per IP server-side.
 *
...
export function registerCompetitionTools(server: McpServer): void {
  server.registerTool(
    "competition_status",
    {
      description: `Get an agent's final AIBTC trading competition standing, with mark-to-current P&L computed locally. The competition has concluded; this reads the historical record.
